...
namespace RatEncounterDesktop.Render
{
    public class PoseableObject
    {
        ...
        private Vector3[] m_posedNormals_keepVAO;
        public Vector3[] PosedVertices { get { return m_posedVertices_keepVAO; } set { m_posedVertices_keepVAO = value; } }
        public Vector3[] PosedNormals { get { return m_posedNormals_keepVAO; } set { m_posedNormals_keepVAO = value; } }
        ...
        public void ResetPose()
        {
            ...
            m_posedNormals_keepVAO = new Vector3[ref_linkObjMesh.PointData.Length];
            ...
            for (int ix = 0; ix < m_posedNormals_keepVAO.Length; ix++) { m_posedNormals_keepVAO[ix] = new Vector3(ref_linkObjMesh.POINT_NORMALS[ix]); }
        }
        ...
        public Triangle3D GetPosedTriangle(int index)
        {
            if (index < ref_linkObjMesh.TriangleCount)
            {
                ...
                tnorms[0] = m_posedNormals_keepVAO[ref_linkObjMesh.PointRef[index * 3 + 0]];
                tnorms[1] = m_posedNormals_keepVAO[ref_linkObjMesh.PointRef[index * 3 + 1]];
                tnorms[2] = m_posedNormals_keepVAO[ref_linkObjMesh.PointRef[index * 3 + 2]];

                Triangle3D tri = new Triangle3D(tpoints);
                tri.Point_Normals = tnorms;
        ...
        {
            ref_linkObjMesh.Render(MainCamera, useTextureSlot, LightSource, LightColor, true, this);
        }
    }
}
