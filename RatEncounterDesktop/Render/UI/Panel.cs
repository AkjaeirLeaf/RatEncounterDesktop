...
namespace RatEncounterDesktop.Render.UI
{
    public class Panel : UIObject
    {
        ...
        public Panel(Vector2 pos, Vector2 size, Color tint, UI_ScalingMode scalingMode)
        {
            Tint = tint;
        ...
        public Panel(Vector2 pos, Vector2 size, Color tint)
        {
            Tint = tint;
            main_panelbody = UIObject.GetRectangle(pos, size);
            geo_obj = new BasicUIGeometry[] { main_panelbody };
            draw_object_bounds = new Vector2[]
            {
                pos, new Vector2(pos.X + size.X, pos.Y), pos + size, new Vector2(pos.X, pos.Y + size.Y)
        ...
        public Panel(Point pos, Size size, Color tint, Kirali.Light.Camera MainCamera)
        {
            Tint = tint;
        ...
        {
            if (GameWindow.CurrentBoundTexture != 0)
            {
                GameWindow.CurrentBoundTexture = 0;
            }
