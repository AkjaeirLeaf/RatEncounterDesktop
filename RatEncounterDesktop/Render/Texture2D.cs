...
namespace RatEncounterDesktop.Render
{
    public partial class Texture2D
    {
        ...
        {
            //Bind Texture
            if (GameWindow.CurrentBoundTexture != handle)
            {
                GL.BindTexture(TextureTarget.Texture2D, handle);
                GameWindow.CurrentBoundTexture = handle;
            }
            GL.BindTexture(target, handle);
        }

        /*
        public virtual void Draw(int x, int y, int x_tile, int y_tile, System.Drawing.Color tint, Camera mainCamera)
        {
        ...
        */
        ...
        public virtual void DrawAsBackground(int world_x, int world_y, double zoom, System.Drawing.Color tint, Kirali.Light.Camera mainCamera)
        {
            // Specifically for tiles and snapped objects
            double screen = ((double)mainCamera.Height / mainCamera.Width);

            double camera_L = mainCamera.position.X -  zoom;
            double camera_R = mainCamera.position.X +  zoom;
            double camera_T = -mainCamera.position.Y - zoom * screen;
            double camera_B = -mainCamera.position.Y + zoom * screen;
